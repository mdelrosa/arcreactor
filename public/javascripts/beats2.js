...
$(document).ready(function() {
  ...
  var switchSongs = function() {
    ...
    if (window.nextSwitchIn <= 0) {
      ...
      for (j=0;j<=bars.length;j++) {
        if (!(bars[j].track.title === window.tracks[window.currentIndex].title)) {
          ...
          if (checkTimbre>(currentTimbre-delta) && checkTimbre<(currentTimbre+delta)) {
            possibleBars.push(bars[j]);
          }
        }
      }
      var pauseThis = document.getElementById(window.trackNames[window.currentIndex].split(' ')[0]);
      pauseThis.pause();
      var nextBar = possibleBars[Math.floor(possibleBars.length*Math.random())];
      window.currentIndex = trackNames.indexOf(nextBar.track.title);
      console.log('currentTrackTitle', nextBar.track.title)
      ...
      console.log(nextBar.start);
      console.log(timeChanger.currentTime);
      timeChanger.currentTime = parseInt(nextBar.start);
